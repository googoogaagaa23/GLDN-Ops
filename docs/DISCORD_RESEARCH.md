...
Research listing-item restrictions in the signed-in EcomSniper Discord server through Chrome Profile 2, preserve the source evidence, and publish only human-reviewed community signals into GLDN Ops Listing Preflight. Official eBay policies are reviewed and published separately as the authoritative baseline.

There is no Discord bot, token, server installation, or background collector. No eBay API is used. Research never sends Discord messages, reactions, edits, or moderation actions.
...
1. Open the EcomSniper server in signed-in Chrome Profile 2.
...
9. Publish those reviewed decisions into the extension rules file.
10. Reload GLDN Ops and run Listing Preflight against a sample that includes a known Block, a known Review, and an unmatched item.
...
The publisher accepts only `asin`, `brand`, or `keyword` rules and only `review` or `block` decisions. Every community rule requires `sourceType: profile2-discord`, a reason, reviewer, review date, and exact Discord source-message URL. Official policy decisions use `sourceType: official-ebay` and an exact official eBay help URL. Raw Discord messages and attachments do not enter the extension package.
...
- No Discord user token, self-bot, browser cookie export, or hidden account access is permitted.
