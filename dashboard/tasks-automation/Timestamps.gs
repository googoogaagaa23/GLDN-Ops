...
function getTaskTimestampRepairStatus() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAME);
  const labels = ['Transaction Defect Rate', 'Late Shipment Rate', 'Tracking Uploaded On Time', 'Cases Closed without seller Resolution'];
  return { version: '3.12.35', spreadsheetId: ss.getId(), sheet: SHEET_NAME,
    metricRows: Object.fromEntries(labels.map((label) => [label, findTaskRowByPrefix_(sheet, label)])),
    editHandlers: ScriptApp.getProjectTriggers().filter((trigger) => trigger.getEventType() === ScriptApp.EventType.ON_EDIT).map((trigger) => trigger.getHandlerFunction()) };
}
...
function handleEdit(e) {
  ...
  const validations = e.range.getDataValidations();
  const formulas = e.range.getFormulas();
  const labels = sheet.getRange(e.range.getRow(), TASK_COL, values.length, 1).getDisplayValues();
  const metricColumns = new Set();
  const editedColumns = new Set();
  let snipingChanged = false;
  for (let r = 0; r < values.length; r++) {
    const row = e.range.getRow() + r;
    for (let c = 0; c < values[r].length; c++) {
      const col = e.range.getColumn() + c;
      if (row < 4 || formulas[r][c]) continue;
      if (col === 12 && isAccountSnipedInputCell(sheet, row, col)) {
        snipingChanged = true;
        continue;
      }
      const checkbox = isCheckboxValidation(validations[r][c]);
      const manualInput = col >= 5 && col <= 10 && Boolean(String(labels[r][0] || '').trim());
      if (!checkbox && !manualInput) continue;
      const cell = e.range.getCell(r + 1, c + 1);
      const value = values[r][c];
      if (checkbox) {
        if (value === true) {
          stampTaskEdit_(cell, now, 'Last checked');
          ...
          // An uncheck is an edit, not evidence that the task was completed again.
          stampTaskEdit_(cell, now, 'Last edited', true);
        }
        logCheckboxEdit(sheet, cell, value, now);
      } else {
        stampManualInput(cell, now);
        if (isManualPercentTrackingCell(sheet, row, col)) metricColumns.add(col);
      }
      editedColumns.add(col);
    }
  }
  if (snipingChanged) updateAccountSnipedTimestamp(sheet, now);
  if (!editedColumns.size) return;
  metricColumns.forEach((col) => updatePerformanceCheckboxFromMetrics_(sheet, col, now));
  updateSessionOnEdit(sheet, now);
  updateProgressCell(sheet);
  editedColumns.forEach((col) => updateComputerTimestamp(sheet, col));
...
function isManualPercentTrackingCell(sheet, row, col) {
  ...
  const label = String(sheet.getRange(row, TASK_COL).getDisplayValue() || '').trim().toLowerCase();
  return ['late shipment rate', 'tracking uploaded on time', 'transaction defect rate', 'cases closed without seller resolution']
    .some((prefix) => label.startsWith(prefix));
}

function stampTaskEdit_(cell, now, label, preserveCompletion = false) {
  const tz = cell.getSheet().getParent().getSpreadsheetTimeZone();
  const human = Utilities.formatDate(now, tz, "MM/dd/yyyy hh:mm:ss a");
  const existing = String(cell.getNote() || '').split('\n').filter((line) => {
    if (preserveCompletion) return !/^(Last edited|Edited epoch):/i.test(line);
    return !/^(Last checked|Value entered|Last edited|Epoch|Edited epoch):/i.test(line);
  }).filter(Boolean);
  const epochLabel = preserveCompletion ? 'Edited epoch' : 'Epoch';
  cell.setNote([`${label}: ${human}`, `${epochLabel}: ${now.getTime()}`, ...existing].join('\n'));
}
...
function stampManualInput(cell, now) {
  ...
  stampTaskEdit_(cell, now, 'Value entered');
}
...
function applyZeroPercentAlert_(sheet, taskText) {
  const row = findTaskRowByPrefix_(sheet, taskText);
  if (!row) return;
...
function updatePerformanceCheckboxFromMetrics_(sheet, col, now) {
  ...
  const metricRows = [
    findTaskRowByPrefix_(sheet, "Transaction Defect Rate"),
    findTaskRowByPrefix_(sheet, "Late Shipment Rate"),
    findTaskRowByPrefix_(sheet, "Tracking Uploaded On Time"),
    findTaskRowByPrefix_(sheet, "Cases Closed without seller Resolution")
  ];
...
function applyLateShipmentRateAlert(sheet) {
  const row = findTaskRowByPrefix_(sheet, "Late Shipment Rate");
  if (!row) return;
...
function applyTrackingUploadedAlert(sheet) {
  const row = findTaskRowByPrefix_(sheet, "Tracking Uploaded On Time");
  if (!row) return;
...
function findTaskRowByPrefix_(sheet, text) {
  const needle = String(text || '').trim().toLowerCase();
  if (!needle || sheet.getLastRow() < 1) return null;
  const matches = sheet.getRange(1, TASK_COL, sheet.getLastRow(), 1).getDisplayValues()
    .map((row, index) => String(row[0] || '').trim().toLowerCase().startsWith(needle) ? index + 1 : null)
    .filter(Boolean);
