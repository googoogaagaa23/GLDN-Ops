...
assert.match(background, /async function openDashboardTab\(options = \{\}\)/);
assert.match(background, /const \{ url, key \} = await getDashboardConfig\(\)/);
assert.match(background, /dashboard\.searchParams\.set\('key', key\)/);
assert.match(background, /const opened = await openTab\(dashboard\.toString\(\), \{ active: options\.active !== false \}\)/);
...
assert.match(popup, /runtimeMessage\(\{ type: 'openDashboard' \}\)/);
